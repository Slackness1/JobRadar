...
STRATEGIES = ["基本面权益", "卖方研究", "量化", "多资产_FOF_衍生品", "固定收益", "相关补充"]


def load_bucket(strategy: str) -> list[dict]:
    """加载一个 bucket 的 jsonl, 过滤掉 relevance < 0.3 的噪声。"""
    path = PILOT_DIR / f"{strategy}.jsonl"
    if not path.exists():
        return []
    records = []
...
def build_synthesis_prompt(bucket_summaries: list[dict], employment: dict) -> str:
    """组 Opus prompt — 6 bucket 摘要 + 就业报告 + 任务说明。"""
    lines = ["# 任务\n"]
    lines.append("你是金融求职赛道分析专家。我给你 6 个 strategy 大类下用 DeepSeek 从小红书抽取的结构化数据 + SAIF MF 学院 2024-2025 真实就业流向。请合成最终 taxonomy:\n")
    lines.append("\n## 你要输出的内容 (按 markdown 章节)\n")
    lines.append("""
### 1. 三维 Taxonomy

按 spec §4.1 的 3 dimension 输出:
- `strategy_type` (6 大类, canonical 已锁): 基本面权益 / 量化 / 固定收益 / 卖方研究 / 多资产_FOF_衍生品 / 相关补充
  - 每个大类下细分 2-6 个 sub_category (从 XHS 数据真出现 + 学生原话验证, 优先选 mention 数高的)
- `industry_focus`: 行业方向 (消费 / TMT / 医药 / 金融 / 周期 / 等), 不锁词表
- `institution_tier`: 平台分层 (一线公募 / 二线公募 / 头部主观私募 / 量化大厂 / 卖方研究所 / 银行理财子 / 保险资管 / 券商资管)

每个细分写 1 行: `<canonical> · <sub_category> · <典型公司 3-5 家> · <学生原话标志词 / 区分点>`。

### 2. 给 SAIF 学生看的 "细颗粒度地图"

把 1 里的细分按 strategy 大类整理成树形, 给学生看哪条路径符合自己。每个细分挂 1-2 条 XHS 学生 verbatim quote 作为佐证。

### 3. Demo 选 10 家公司

挑 10 家公司, 覆盖至少 4 个 strategy 大类, 满足以下条件:
- 在 XHS 数据 + 就业报告中均有 mention (双重 ground truth)
- 跨越 institution_tier (顶配公募 / 二线公募 / 头部私募 / 量化大厂 / 卖方所 / 资管子 / 险资 之类)
- 配 1 句话 demo pitch (为什么选它, 它能区分哪类 persona)

### 4. 区分力 sanity check

简短 1-2 段, 评估你给的 taxonomy 能否在以下 5 维都做出区分:
(a) P1 公募基本面 vs P6 量化私募 — strategy 主轴
(b) P1 公募 vs P3 私募 — institution_tier
(c) P1 买方 vs P2 卖方 — strategy 内部
(d) 跨专业 P3 (理工→金融) 友好度
(e) 隐藏亮点挖掘 (P1 deal size / P2 头部 broker / P6 sharpe ratio 等)
...
def main() -> int:
    ...
    prompt = build_synthesis_prompt(bucket_summaries, employment)
