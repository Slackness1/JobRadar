...
A **second, independent path** parallel to the existing parse → confirm → `generate_recommendations` → `/resume-copilot?sessionId=X` flow. Plan-mode tackles the *resume writing* problem instead (one-bullet-at-a-time, evidence-gated, structurally reverse-hallucinated) — Claude-Code-style "one tool call per turn" for resume building. **The two paths do not share state**; deciding when/how to converge is open work. Migration is `alembic 0003_session_plan_json` adding `ResumeCopilotSession.plan_json: JSON`.
