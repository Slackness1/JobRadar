...
**Plan-mode resume building** (2026-05-13, `app/services/resume_copilot/{plan,plan_turn,tag_extractor}.py` + `agent/builder.py`):

A **second, independent path** parallel to the existing parse → confirm → `generate_recommendations` → `/resume-copilot?sessionId=X` flow. Plan-mode tackles the *resume writing* problem instead (one-bullet-at-a-time, evidence-gated, structurally reverse-hallucinated) — Claude-Code-style "one tool call per turn" for resume building. The two URLs/UIs stay separate, but a **one-way bridge** mirrors finalized plan content into `ResumeConfirmedProfile.profile_json` so the next `/generate` run picks up the better-written bullets — see `plan_sync.py` below. Migrations: `alembic 0003_session_plan_json` (adds `ResumeCopilotSession.plan_json: JSON` + `plan_status`), `alembic 0004_recommendations_stale` (adds `recommendations_stale: int` flag).

- `plan.py` — **pure data + pure functions, no DB no LLM no I/O**. Defines `PlanStatus` × `ItemStatus` enums, `PlanItem` / `Evidence` / `EvidenceTag` Pydantic models, and the transition table. `apply_action(plan, action)` validates every state jump — `pending → finalized` 一步登天 raises `IllegalTransition`. `audit_draft(draft_text, evidence) -> AuditResult` is **the reverse-hallucination gate**: every concrete number / tech stack name / leadership verb in the draft must trace back to an `EvidenceTag` (`metric` / `tech` / `scope` / `role` / `verb_subject`). Blocking risk flags reject the write at the data layer — by the time a draft hits storage, hallucinations are structurally impossible.
- `tag_extractor.py` — regex extraction of `metric` / `tech` / `scope` / `role` / `verb_subject` / `outcome` tags from raw evidence text, feeding `audit_draft`.
- `agent/builder.py` — **one LLM call → one `AgentAction`** (5 actions: `ask` / `ready_to_write` / `write` / `drop` / `block`). System prompt nails the铁律: `draft_text` can't reference numbers/tech/leadership not in evidence; one JSON per turn no markdown; one focused question per `ask`. Pydantic schema validates the response; on `ValidationError` does **1 retry** then **falls back to `ask`**. Audit failure inside `write` is auto-converted to an `ask` so users never see the audit exception — the agent just asks for the missing evidence instead.
- `plan_turn.py` — turn handler that wires `agent/builder` → `apply_action` → persist. Initial plan is built from a **fixed YAML template + `parsed_counts`** (零 LLM 零 hallucination): deterministic items keyed by `ItemKind` (self_intro / education / internship / project / campus_activity / skill / award) and pre-counted from the parsed resume.
- `plan_sync.py` — **one-way bridge into legacy recommendation flow**. After every `apply_action` that newly transitions any item to FINALIZED, both call sites (`plan_turn.run_plan_turn` and the raw `/plan/actions` endpoint via `_maybe_sync_plan_to_profile` helper) call `sync_plan_to_profile(new_plan, profile) -> profile`. Pure function: maps SELF_INTRO → `candidate_summary`, EDUCATION[i] → `education[i].highlights`, INTERNSHIP[i]/PROJECT[i] → `internships[i]/projects[i].bullets` (collected from FINALIZED children sorted by order). SKILL/AWARD/CAMPUS_ACTIVITY are **skipped in v1** (profile schema mismatch). Conflict policy: latest plan wins; structural fields (company/role/dates) are never touched. Side effects: writes back to `ResumeConfirmedProfile.profile_json` and flips `session.recommendations_stale = 1`. Workspace shows a yellow banner gated by `recommendations_stale && has_recommendations` with a "重新推荐" button that calls `regenerateFromSyncedProfile` (skips `putConfirmedProfile` to avoid clobbering the synced bullets with React's parsed-profile state). `/generate` clears the flag.
- **Router endpoints** (`app/routers/resume_copilot.py`): `POST /api/resume-copilot/sessions/{id}/plan/start`, `GET /api/resume-copilot/sessions/{id}/plan`, `POST /api/resume-copilot/sessions/{id}/plan/approve`, `POST /api/resume-copilot/sessions/{id}/plan/turn`. All gated by `_get_session_or_404` + demo-read-only-guard.
- **Frontend route** `/resume-copilot/plan?sessionId=X` (`resume-copilot-web/app/resume-copilot/plan/page.tsx` → `components/resume-copilot/plan/PlanPanel.tsx`, 368 lines). Left rail: item tree grouped by `parent_id`, status chips terracotta-themed (8 colors keyed to `ItemStatus`). Right pane: selected item detail + chat composer + draft preview.
- **Test floor**: 97 tests across 5 files (`test_resume_plan.py` 29 / `test_resume_plan_router.py` 15 / `test_resume_plan_builder.py` 12 / `test_resume_plan_turn.py` 6 / `test_resume_tag_extractor.py` 18 / `test_resume_plan_agent.py` 17 — agent-loop). Plus `backend/scripts/smoke_plan_turn.py` real-LLM smoke (DeepSeek, currently green).
...
> - **09:00 Asia/Shanghai** — `_daily_tier_crawl_job` runs the tier orchestrators sequentially with error isolation per block. Populates `company_crawl_logs`. Slow, ~30-40 min, Playwright-heavy. Each block wrapped so one failure doesn't stop the others; a parent `CrawlLog` row with `source='tier-crawl'` aggregates the run. **10 blocks (Phase order)**: `internet` → `state_owned` → `securities` → `consumer_foreign` → `insurance` (Phase 6) → `funds` (Phase 6) → `pe_vc` (Phase 7) → `hedge_funds` (Phase 9) → `foreign_ibs` (Phase 10) → `consumer_foreign_workday` (Phase 13, 阿斯利康 — single thin wrapper around `foreign_ibs_tier_crawler._fetch_workday_filtered` stamped with `source='consumer_foreign_official'`). foreign_ibs alone runs ~4.5 min (Citi+MS via Workday `searchText` filter; full pagination of 2000 global jobs would be ~10 min).
...
**`jobs_company_match` second-level lookup** (added 2026-05-12) — for `enumerate` entries whose parent portal multiplexes multiple subsidiaries (e.g. 平安集团 portal 一锅烩 829 jobs / `source=bank_official`, but `jobs.company` is already split into 平安寿险 / 平安产险 / 平安证券 / 平安银行 / 平安人寿). `coverage.py:_resolve_status` does a **two-tier** check: first try `jobs_company_match` keyword list against `jobs` table (7-day window) — if hit, use that count; otherwise fall back to the original `company_crawl_logs` query. yaml example: `平安银行: jobs_company_match: [平安银行]`; `中国平安: jobs_company_match: [平安寿险, 平安产险, 平安人寿, 平安财产保险, 平安养老, 平安健康]`; `平安证券: jobs_company_match: [平安证券]`. Effect: 银行/保险/券商 3 个赛道同时把平安子实体的真实 active 数算出来，而不是被 portal 一锅烩淹没。
...
- `/review-queue` surfaces jobs that the crawler LLM left ambiguous (`quality_label IN ('', 'low_signal')` OR `track_predicted == ''`) in the last 30 days. UI is wireframe variant C (high-density table with batch select). Backend endpoints: `GET /api/review-queue`, `POST /api/review-queue/{id}/{approve|reject|retrack}`, `POST /api/review-queue/batch`. `track_predicted` (freeform LLM output) is re-bucketed into 3 coarse columns (FinTech / 纯金融 / 其他) for kanban view — see `_bucket()` mapping in `review_queue.py`. Retrack writes a real `track_predicted` value (any of 8 keys: internet/banks/securities/funds/pe_vc/insurance/state_owned/consumer_foreign/FinTech).
  ...
  **Teacher OCR bridge** (2026-05-12): the same `/review-queue` page also surfaces `JobDraft` rows submitted via the `/teacher` quick-entry flow (链接 / OCR / 文本三模). `GET /api/review-queue` returns `teacher_drafts: [...]` + `teacher_pending_total`; UI adds an `✏️ 教师录入 N` tab alongside the FinTech / 纯金融 / 其他 kanban tabs. Proxy endpoints `POST /api/review-queue/teacher-drafts/{id}/{approve|reject}` call `teacher_entry._promote_draft_to_job` + `scorer.score_all_jobs` in-process — on approve, `jobs.source='teacher_entry'`, `quality_label='good'`, dropping the row straight into the recommendation pool. Proxy is unauthenticated because it's same-origin/same-process (the underlying `/api/teacher-entry/admin/*` endpoints still require admin token for cross-origin callers).
...
| `/resume-copilot?sessionId=X` | `public-resume-copilot.tsx` | Workspace (legacy recommendation flow). `sessionId=1` shows `<DemoBanner/>` and disables chat composer + apply-rewrite buttons. |
| `/resume-copilot/plan?sessionId=X` | `app/resume-copilot/plan/page.tsx` → `components/resume-copilot/plan/PlanPanel.tsx` | **Plan-mode** resume builder (2026-05-13). Item tree (left) + selected-item chat + draft preview (right). Each turn = one structured `AgentAction` (ask / ready_to_write / write / drop / block). Independent path from `/resume-copilot` — does not share state with the recommendation flow. |
