...
- 多资产_FOF_衍生品: 资管 FOF, 自营 FOF, 财富 FOF, 结构化衍生品, 权益/自营/做市 交易台, 基金产品运营·中后台
- 相关补充: PEVC 投资研究·投后支持
...
- 股票/权益交易岗、自营交易、做市交易、交易执行(非量化策略研发,即不写 alpha/因子/回测) → 多资产_FOF_衍生品 (下游选"权益/自营交易员");
  注意: 交易岗 ≠ 研究岗 ≠ 中后台,带"交易/做市/自营盘口"且非固收的一律走这里, 不要因为是公募/券商就归基本面权益研究员或中后台
...
补充硬规则 (r4 — 跨厂商判官审出的高频误判, 2026-06-19):

一、卖方研究按板块, 板块不在候选不得硬塞
- "卖方研究员·消费医药周期"只收消费/食品饮料/医药/农林牧渔/社服等大消费 + 医药板块。
- 机械/军工/有色/钢铁/煤炭/建材/化工/石油石化/电新/大宗商品/期货商品研究 = 周期上游/大宗, **不属于消费医药周期**;
  若候选里无对应板块桶, 归"卖方研究员·宏观策略"或置 confidence ≤ 0.45 + evidence_path=low_signal, **绝不硬塞消费医药周期**。
- 卖方金工/金融工程/量化组归卖方近邻或 low_signal, 不归买方量化。

二、FOF 必须有"母基金/子基金遴选 + 资产配置"实质职责
- 企业年金/职业年金 受托、年金销售、年金管培、养老金业务管培、基金选品(无配置职责)、产品销售 = **不归任何 FOF 桶**。
- FOF(自营/资管/财富)的硬门槛: JD 必须出现 母基金/子基金/基金遴选/管理人尽调/MOM/资产配置/组合构建/FOF 投研 至少一项;否则归中后台/销售/管培近邻或 low_signal。

三、量化"研究" vs "开发/执行/Infra" 分清
- Execution Technology/执行技术/订单执行系统/做市系统 = "量化开发 QD"或工程, **不归量化研究员·中频/高频**(研究 = 策略/因子/收益预测/回测)。
- AI Infra/算力架构/ML 平台/底层框架 = 工程, 不归"AI 量化工程师"(后者必须有 alpha/因子/交易/组合投资目标)。

四、前台 vs 中后台 严格区分
- 内核/质控/合规审查/风控审批/登记清算/估值核算/运营支持 = 中后台, **不归** 债券承做/投行 IBD/交易员 等前台桶。
- 交易员/投资经理/研究员 = 前台投研交易, **不归** 公募基金综合中后台。
- "登记清算/份额登记/TA/运营报表"归"基金产品运营·中后台", 不归"公募基金综合中后台(合规/法务/行政)"。

五、银行: 总行 vs 分行
- "分行/支行/自贸区分行 培训生/管培" = 分行基层, **不归"银行总行管培"**(后者限总行编制管培)。无法确定层级时不归总行管培。

六、风险管理·投资监督 限投资口径
- "投资监督/投资组合风控/合规监督/风险计量(投资端)" 才归"风险管理·投资监督"。
- 支付风控/信贷风控/反欺诈/精算定价/业务风控 = 业务侧风控, **不归投资监督**(无候选则 low_signal)。

补充硬规则 (r5 — 2026-06-19 二轮判官审出的残留高频误判):

七、卖方 vs 买方 一刀切(最易错)
- 出现"研究所/证券研究所/经济与金融研究院/研究发展部(券商)"或岗名带"卖方"= **卖方研究**;
  即使是"行业研究/固收研究", 也归卖方对应桶, **不归公募权益/公募行业/非公募买方/卖方固收以外的买方桶**。
- 出现"持仓/入库/投资经理/组合/基金经理助理/内部研究服务投资决策"= **买方研究**, 不归任何"卖方研究员·*"。
- 判不准买卖方时, 看服务对象: 服务券商外部机构客户=卖方;服务本机构投资决策=买方。仍不清 → confidence ≤ 0.45 + low_signal。

八、管培生/管理培训生/轮岗生 默认不进专业投研桶
- 标题为"管培生/管理培训生/储备干部/轮岗培养生"且 JD 未明确锁定某专业投研职能(如未写明专做利率研究/信用研究/量化策略)= **置 low_signal, 不归任何专业 sub_cat**。
- "绿色金融方向/普惠方向/数字金融方向管培"= 产业/产品创新条线, **不归利率宏观/信用研究等投研桶**。
- 无 JD 或 JD 仅一句宽泛描述、无法确认具体职能的 = low_signal, 绝不按标题关键词硬塞。

九、战略/战投/咨询/产业研究(非券商研究所)≠ 卖方研究
- "战略实习生/战投/企业战略/产业研究(互联网或实业公司)/管理咨询产业研究/投行产业研究"= 战略或投行口径, **不归"卖方研究员·*"**(卖方研究限券商研究所对外发布研报)。

十、中后台职能再确认(继续清前台漏网)
- "ETF Pricing/定价运营/份额登记/基金会计/估值核算/清算"= 中后台运营, **不归量化研究/交易/承做等前台**。
- "项目管理助理/档案整理/投后项目管理(运营口)/反洗钱/合规审查"= 中后台或合规, **不归 PE/VC 投研投后、DCM 承做等前台**。
- "数据分析助理/IT/数据开发"= 技术运营, **不归公募基金综合中后台(合规/法务/行政)**(后者限合规/法务/行政职能)。

十一、互联网/实体行业分析师 ≠ 金融投研
- "餐饮/消费品/电商 行业生态分析、商业分析师、业务分析(互联网或实体公司内部)"= 实体商业分析, **不归公募/资管/卖方 任何金融投研桶**(金融投研以二级市场标的研究 + 服务投资决策为内核)。

十二、"权益/自营交易员" 是交易台岗(此前无归宿,易被误塞研究/中后台)
- 选"权益/自营交易员"的条件: 岗位核心是**下单/执行/持仓盯盘/做市报价/自营盘交易**, 标题或 JD 含 股票交易/权益交易/自营交易/做市/交易执行/交易员(非固收、非量化策略研发)。
- 与近邻区分: 写 alpha/因子/信号/回测/策略研发 → 量化(不选本桶);券商研究所对外研报 → 卖方研究;面向机构客户卖产品 → 机构销售;
  固收/债券/利率 交易 → 固收交易员(不选本桶);估值/清算/登记/风控审批 → 中后台(不选本桶)。
- 该桶 KB 暂空, 仅按本规则判定即可, 不要因候选文本里没有它的描述就不选。
...
# 确定性"模糊岗踢出池"守卫 (2026-06-19 老师拍板: 学院要"到位", 宁缺勿滥)
# 比纯 prompt 规则可靠 — 直接在入口拦掉, 不烧 LLM。
_GENERIC_TRAINEE = re.compile(r"管培生|管理培训生|储备干部|储备人才|轮岗培养|培训生计划|管培计划|trainee", re.I)
_SPECIALTY_ANCHOR = re.compile(
    r"研究|投研|投资|量化|因子|alpha|交易|做市|承做|承销|投行|并购|风控|风险|估值|清算|登记|"
    r"FOF|衍生品|结构化|信用|利率|宏观|策略|销售|投后|尽调|固收|权益|资产配置|组合"
)


def _is_ambiguous_offtarget(job_dict: dict[str, Any]) -> bool:
    """天生模糊岗 → 不属于任何细分, 踢出推荐池(返 None)。
    两类: ① 空/极短 JD 且标题无专业锚(无从判定对口);② 泛管培生且通篇无专业职能锁定。"""
    jd = (job_dict.get("job_duty", "") + " " + job_dict.get("job_req", "")).strip()
    title = job_dict.get("job_title", "")
    if len(jd) < 30 and not _SPECIALTY_ANCHOR.search(title):
        return True
    if _GENERIC_TRAINEE.search(title) and not _SPECIALTY_ANCHOR.search(title + " " + jd):
        return True
    return False
...
def enrich_job_sub_cat(job: Job) -> dict[str, Any] | None:
    ...
    if _is_ambiguous_offtarget(job_dict):
        return None
