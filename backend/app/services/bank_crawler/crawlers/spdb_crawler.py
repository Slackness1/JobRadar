...
class SPDBCrawler(BaseBankCrawler):
    """浦发银行招聘爬虫

    官网: https://job.spdb.com.cn/
    """
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            logger.error(f"[spdb] Connection test failed: {e}")
            return False
    ...
    def fetch_jobs(self, stage: str = "campus") -> List[Dict]:
        ...
        try:
            ...
            resp = self._get(url)
            if not resp:
                logger.error(f"[spdb] Failed to fetch {url}")
                return jobs

            soup = self._parse_html(resp.text)

            # 查找职位列表
            job_elements = soup.select(".job-list-item, .position-item, .job-item, tr.job-row")
            ...
            for job_elem in job_elements:
                try:
                    ...
                    logger.warning(f"[spdb] Failed to parse job element: {e}")
                    continue

        except Exception as e:
            logger.exception(f"[spdb] Failed to fetch jobs: {e}")

        logger.info(f"[spdb] Fetched {len(jobs)} jobs for stage {stage}")
        return jobs
