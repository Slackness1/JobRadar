...
class NBCBCrawler(BaseBankCrawler):
    """宁波银行招聘爬虫

    官网: https://zhaopin.nbcb.cn/
    """
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            logger.error(f"[nbc] Connection test failed: {e}")
            return False
    ...
    def fetch_jobs(self, stage: str = "campus") -> List[Dict]:
        ...
        try:
            ...
            resp = self._get(url)
            if not resp:
                logger.error(f"[nbc] Failed to fetch {url}")
                return jobs
            ...
            for job_elem in job_elements:
                try:
                    ...
                    logger.warning(f"[nbc] Failed to parse job element: {e}")
                    continue

        except Exception as e:
            logger.exception(f"[nbc] Failed to fetch jobs: {e}")

        logger.info(f"[nbc] Fetched {len(jobs)} jobs for stage {stage}")
        return jobs
