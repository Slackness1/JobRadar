...
class ICBCrawler(BaseBankCrawler):
    """中国银行招聘爬虫

    官网: https://campus.icbc.com.cn/
    """
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            logger.error(f"[icbc] Connection test failed: {e}")
            return False
    ...
    def fetch_jobs(self, stage: str = "campus") -> List[Dict]:
        ...
        try:
            ...
            resp = self._get(url)
            if not resp:
                logger.error(f"[icbc] Failed to fetch {url}")
                return jobs
            ...
            for job_elem in job_elements:
                try:
                    ...
                    logger.warning(f"[icbc] Failed to parse job element: {e}")
                    continue

        except Exception as e:
            logger.exception(f"[icbc] Failed to fetch jobs: {e}")

        logger.info(f"[icbc] Fetched {len(jobs)} jobs for stage {stage}")
        return jobs
