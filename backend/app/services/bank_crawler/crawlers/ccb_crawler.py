...
class CCBCrawler(BaseBankCrawler):
    """建设银行招聘爬虫

    官网: https://job.ccb.com/
    """
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            logger.error(f"[ccb] Connection test failed: {e}")
            return False
    ...
    def fetch_jobs(self, stage: str = "campus") -> List[Dict]:
        ...
        try:
            ...
            resp = self._get(url)
            if not resp:
                logger.error(f"[ccb] Failed to fetch {url}")
                return jobs
            ...
            for job_elem in job_elements:
                try:
                    ...
                    logger.warning(f"[ccb] Failed to parse job element: {e}")
                    continue

        except Exception as e:
            logger.exception(f"[ccb] Failed to fetch jobs: {e}")

        logger.info(f"[ccb] Fetched {len(jobs)} jobs for stage {stage}")
        return jobs
