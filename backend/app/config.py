...
# 学生交互"低延迟"专线(2026-06-15):推荐精排/narrative/意图/子类预勾走这条。
# 默认回落到 RESUME_COPILOT_LLM_*(不设=与之前行为一致);设了就独立指向
# DeepSeek 官方直连——因为 OpenCode 中转对并发重度限流(实测单调用 ~26s vs 官方 ~1.4s),
# 凡"学生坐屏幕前等"的调用必须走快通道。后台批处理(CRAWLER/ENRICH)仍走中转。
INTERACTIVE_LLM_BASE_URL = os.environ.get("INTERACTIVE_LLM_BASE_URL", RESUME_COPILOT_LLM_BASE_URL)
INTERACTIVE_LLM_API_KEY = os.environ.get("INTERACTIVE_LLM_API_KEY", RESUME_COPILOT_LLM_API_KEY)
INTERACTIVE_LLM_MODEL = os.environ.get("INTERACTIVE_LLM_MODEL", RESUME_COPILOT_LLM_MODEL)
INTERACTIVE_LLM_TIMEOUT_SECONDS = _get_int_env("INTERACTIVE_LLM_TIMEOUT_SECONDS", 60)
