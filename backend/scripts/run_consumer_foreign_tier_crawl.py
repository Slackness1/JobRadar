...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Crawl official jobs for consumer / foreign tier companies.")
    parser.add_argument("--include-tiers", default="", help="Comma-separated tiers to include")
...
def main() -> None:
    ...
    include_tiers = set(_split_csv(args.include_tiers))
    ...
    if include_tiers:
        targets = [target for target in targets if target.tier in include_tiers]
    ...
    prefix = args.output_prefix or f"consumer_foreign_tier_crawl_{timestamp}"
    ...
    db = SessionLocal()
    try:
        results = crawl_consumer_targets(
            db,
            targets,
