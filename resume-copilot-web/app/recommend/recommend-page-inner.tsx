...
/**
 * RecommendPageInner — 「推荐工作台」路由的 sessionId 解析层 (Phase G 子项①).
 *
 * 解析约定对齐现有简历工作台:`?session=N` query param;缺省时回退到会话列表
 * 第一条,再退到 DEMO_SESSION_ID(只读 demo 会话). 解析完直接把 sessionId 下传
 * 给 RecommendWorkspaceShell. 全程 setState 仅在 async 回调内,render 纯净.
 */
...
import { RecommendWorkspaceShell } from '@/components/resume-copilot/workspace/recommend-agent/RecommendWorkspaceShell';

export function RecommendPageInner() {
  ...
  useEffect(() => {
    ...
    listResumeCopilotSessions()
      .then((items) => {
        ...
        router.replace(`/recommend?session=${firstId}`);
  ...
  return <RecommendWorkspaceShell sessionId={sessionId} />;
}
