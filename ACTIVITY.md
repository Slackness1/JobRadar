...
- **用户体验变化**:修复前 6 个里 4 个顶级实习被压成腰部(靠大模型常识临场救场才推对);修复后中信/中金/九坤/易方达正确识别为头部,带动 3 个 persona 的匹配档从次头部上修到头部,和人工标注完全吻合。**6 个投研 persona 现 5 准 1 方向性偏严(那 1 个其实是简历实习档真到次头部,判断站得住)。**
- **测试**:41 项后端测试绿;6 persona 真调 DeepSeek Pro 复跑通过。结论:档次定位对投研简历可用于 demo。全在分支,prod 未动。
