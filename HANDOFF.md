# HANDOFF（已废弃 — 2026-05-26）

> ⚠️ **本文件已废弃，不再维护**。
>
> 原设计是"上一次会话写给下一次的速记"，但项目已演变为 5 个并行 Claude 会话同时开发，单一 HANDOFF 无法承载多线程进度。
>
> **新会话冷启动改读**：
> 1. `CLAUDE.md` — 静态架构与规则
> 2. `WORKTREE_STATUS.md` — 当前 5 个会话各自在干什么
> 3. `ACTIVITY.md` — 最近 14 条交付日志（追加式，按倒序）
>
> 以下内容仅作历史归档保留，**不要再读它来决定下一步该做什么**。
